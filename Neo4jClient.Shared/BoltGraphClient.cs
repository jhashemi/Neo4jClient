...
namespace Neo4jClient
{
    ...
    public partial class BoltGraphClient : IBoltGraphClient, IRawGraphClient, ITransactionalGraphClient
    {
        ...
        public BoltGraphClient(string uri, string username = null, string password = null, string realm = null, IDriver driver = null)
        ...
        /// <inheritdoc />
        [Obsolete(NotValidForBolt)]
        public NodeReference<TNode> Create<TNode>(TNode node, IEnumerable<IRelationshipAllowingParticipantNode<TNode>> relationships, IEnumerable<IndexEntry> indexEntries) where TNode : class
        {
        ...
        {
            var token = configuration == null
                ? GetAuthToken(username, password, realm)
                : GetAuthToken(configuration.Username, configuration.Password, configuration.Realm);

            var driver = Driver ?? 
                GraphDatabase.Driver(uri, token, Config.DefaultConfig);
            Driver = driver;
